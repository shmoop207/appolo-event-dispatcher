...
exports.EventDispatcher = exports.RoutingKeysSymbol = exports.CallbacksSymbol = void 0;
...
class EventDispatcher {
        ...
        if (Array.isArray(event)) {
            if (options && options.saga) {
                ...
                for (let i = 0; i < event.length; i++) {
                    results[i] = [];
                    this.on(event[i], util_1.Util.sagaFn(results, i, fn, scope), null, options);
                }
            }
            else {
    ...
    once(event, fn, scope, options = {}) {
        ...
        return new Promise((resolve, reject) => {
            this.on(event, util_1.Util.timeoutFn(options.timeout, resolve, reject), scope, Object.assign(Object.assign({}, options), { once: true }));
        });
    }
    bubble(event, scope) {
